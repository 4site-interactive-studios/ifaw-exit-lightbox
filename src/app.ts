...
export class App {
  ...
  private overlayID = "ifaw-" + Math.random().toString(36).substring(7);

  private overlay: HTMLElement;

  constructor() {
    if (this.isDebug()) {
      console.log("IFAW Exit Lightbox: Debug mode is on");
    }
    if (!this.shouldRun()) {
      if (this.isDebug()) console.log("IFAW Exit Lightbox Not Running");
      return;
    }
  ...
  private shouldRun(): boolean {
    ...
    const hideSignUpForm = crumbs.get(this.options.cookie_name); // Get cookie
    return (
      !hideSignUpForm &&
      !this.isBlacklisted() &&
      this.isWhitelisted() &&
  ...
  private run() {
    ...
    const lang = region.textContent ?? "US";
  ...
  open() {
    const hideCookie = crumbs.get(this.options.cookie_name); // Get cookie
  ...
  submit(e: Event) {
    ...
    axios
      .get("https://api.ifaw.org/api/funnel/signup")
      .then((response: any) => {
        ...
        axios(options)
          ...
          .catch((response: any) => {
            ...
            button.classList.remove("loading");
            alert(
              "Error while subscribring. Please check your e-mail address."
            );
            if (this.isDebug()) console.log(response);
          });
      });
  ...
  success() {
    ...
    const form_desc = document.querySelector(
      "#" + this.overlayID + " .form-desc"
    ) as HTMLElement;
    const title_small = document.querySelector(
      "#" + this.overlayID + " .small-title"
    ) as HTMLElement;
    const title = document.querySelector(
      "#" + this.overlayID + " .title"
    ) as HTMLElement;
    const subtitle = document.querySelector(
      "#" + this.overlayID + " .subtitle"
    ) as HTMLElement;
    const subtitle_small = document.querySelector(
  ...
  isWhitelisted() {
    ...
    if (this.options.whitelist.length) {
      ...
      ret = false;
      this.options.whitelist.forEach((test: string) => {
        if (this.isDebug()) console.log("Checking Whitelist", test);
        if (url.match(new RegExp(test))) ret = true;
      });
    }
    if (this.isDebug()) console.log("Is Whitelisted?", ret);
    return ret;
  }
  isBlacklisted() {
    let ret = false;
    if (this.options.blacklist.length) {
      const url = window.location.pathname;
      this.options.blacklist.forEach((test: string) => {
        if (this.isDebug()) console.log("Checking Blacklist", test);
        if (url.match(new RegExp(test))) ret = true;
      });
    }
    if (this.isDebug()) console.log("Is Blacklisted?", ret);
    return ret;
  }
  isBetweenDates() {
    let ret = true;
    ...
    if (this.options.dates.length) {
      const now = new Date();
      const start = new Date(this.options.dates[0]);
      const end = new Date(this.options.dates[1]);
      if (now < start || now > end) {
        ret = false;
      }
    }
    if (this.isDebug()) console.log("Is betwen Dates?", ret);
    return ret;
  }
